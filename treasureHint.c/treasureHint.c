...
int main(int argc, char *argv[]) {
    ...
    if (argc == 5 && strcmp(argv[3], "load") == 0) {
        loadGame(argv[4]);  // Load the game only if the player is the owner
    } else {
        initializeGame();  // Initialize the game if no load command is provided
    }

    // Handle leaderboard display if requested
